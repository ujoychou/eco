...
void Thread::Impl::join()
{
	if (m_handle != -1)
	{
		...
		if (WAIT_OBJECT_0 != dw)
		{
			ECO_LOG(error, "join thread") << this_thread::id_string()
				<< '.' << m_name << ": " << ::GetLastError();
		}
		m_handle = -1;
	}
...
unsigned __stdcall work(IN void* tdata)
{
	...
	{
		ECO_LOG(error, "thread func") << this_thread::id_string()
			<< '.' << impl.m_name << ": " << e.what();
	}
	return 0;
}
...
void Thread::Impl::run(thread_func& func, IN const char* name)
{
	...
	if (m_handle == 0)
	{
		ECO_THROW(::GetLastError()) << "create win thread error!";
	}
	...
	if (result == -1)
	{
		ECO_THROW(::GetLastError()) << "resume win thread error!";
	}
}
