...
////////////////////////////////////////////////////////////////////////////////
MD5::MD5(const std::string& msg)
{
	make(msg.c_str(), msg.size());
}
MD5::MD5(const char* msg, const size_t size)
{
	make(msg, size);
}
...
////////////////////////////////////////////////////////////////////////////////
void MD5::make(const char* msg, const size_t size)
{
	m_finished = false;
...
const char* MD5::result() 
{
	for (size_t i = 0, j = 0; i < 16; ++i)
	{
		...
		int a = t / 16;
		int b = t % 16;
		m_result[j++] = HEX_NUMBERS[a];
		m_result[j++] = HEX_NUMBERS[b];
	}
	return m_result;
}
...
void MD5::init(const byte* input, size_t len) {
	...
	if ((m_count[0] += ((bit32)len << 3)) < ((bit32)len << 3)) {
		++m_count[1];
	}
	m_count[1] += ((bit32)len >> 29);
...
void MD5::transform(const byte block[64]) {
	...
	bit32 a = m_state[0], b = m_state[1], c = m_state[2], d = m_state[3], x[16];
	decode(block, x, 64);
