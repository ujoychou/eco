thread_local std::string this_msg;
class TestLocal
{
public:
    inline TestLocal()
    ...
    inline ~TestLocal()
    {
        printf("TestLocal destruct \n");
    }

    inline void show()
    {
        printf("show hsow hsow hsow show show show \n");
    }
};

thread_local TestLocal this_test;
///////////////////////////////////////////////////////////////////////////////
class ErrorX : public std::exception
{
public:
    inline ErrorX(int id, const char* fmt, ...) : id(id)
    {
        va_list args;
        va_start(args, fmt);
        this->message().resize(512);
        int size = vsnprintf(&this->message()[0], 512, fmt, args);
...
private:
    virtual const char* what() const _GLIBCXX_NOTHROW override
    {
        ...
        {
            this->message() += " #";
            this->message() += std::to_string(this->id);
        }
    ...
    {
        //if (this_msg == 0) { this_msg = new std::string(); }
        return this_msg;
    }

    int id;
};
#define KSXX_THROW(...) throw ErrorX(__VA_ARGS__)
...
#define ECO_MSG_LEVEL(level, title, ...) \
    ...
    eco::log::Log(level, __FILE__, __LINE__, title, __VA_ARGS__, NULL)

// logging message with title.
#define ECO_MSG(level, title, ...) \
    eco::log::level > eco::log::get().level() ? 0 : \
    eco::log::Log(eco::log::level, title, __FILE__, __LINE__, __VA_ARGS__, NULL)

// logging message without title.
#define ECO_LOG(level, ...) ECO_MSG(level, 0, __VA_ARGS__)
