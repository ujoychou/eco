...
public:
	...
	*/
	...
	@return payload_len_end==0: size not enough.
	*/
	inline eco::Result decode_websocket(
		OUT uint32_t& len_end,
		OUT uint8_t& mask,
		OUT uint32_t& message_size,
		IN  const char* byte,
		IN  uint32_t size) const
	{
		uint8_t  fin = 0;
		uint8_t  frame = 0;
		uint64_t payload_len = 0;
		len_end = decode_websocket(fin, frame, payload_len, mask, byte, size);
	...
	inline bool decode_websocket(IN eco::String& byte) const
	{
		uint8_t mask = 0;
		uint32_t size = 0;
		uint32_t pos = 0;
		if (eco::ok != decode_websocket(
			pos, mask, size, byte.c_str(), byte.size()))
		{
			return false;	// eco::fail means "pos==0", byte not enough size.
		}
		if (mask)
		{
...
public:
		...
		IN const uint32_t size) const override
	{
		...
		uint32_t pos = 0;
		auto res = WebSocket::decode_websocket(pos, mask, msg_size, byte, size);
		...
		if (mask)
		{
			...
			if (size < ver_end) return eco::fail;
			char masking_key[4] = { 0 };
			eco::copy(masking_key, pos, &byte[pos], size_ws_mask_key);
